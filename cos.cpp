...
static SP<Desktop::Rule::CWindowRule>
makeWindowRule(const std::string &name, const Desktop::Rule::eRuleProperty prop,
...
void configureWindow(PHLWINDOW pWindow) {
  static auto *const PSIZEX =
      (Hyprlang::STRING const *)HyprlandAPI::getConfigValue(
          PHANDLE, "plugin:hyprwinwrap:size_x")
          ->getDataStaticPtr();
  static auto *const PSIZEY =
      (Hyprlang::STRING const *)HyprlandAPI::getConfigValue(
          PHANDLE, "plugin:hyprwinwrap:size_y")
          ->getDataStaticPtr();
  static auto *const PPOSX =
      (Hyprlang::STRING const *)HyprlandAPI::getConfigValue(
          PHANDLE, "plugin:hyprwinwrap:pos_x")
          ->getDataStaticPtr();
  static auto *const PPOSY =
      (Hyprlang::STRING const *)HyprlandAPI::getConfigValue(
          PHANDLE, "plugin:hyprwinwrap:pos_y")
          ->getDataStaticPtr();
  ...
  if (px + sx > 100.f) {
    Log::logger->log(
        Log::WARN,
        "[hyprwinwrap] size_x (%d) + pos_x (%d) > 100, adjusting size_x to %d",
        sx, px, 100.f - px);
    sx = 100.f - px;
  }
  if (py + sy > 100.f) {
    Log::logger->log(
        Log::WARN,
        "[hyprwinwrap] size_y (%d) + pos_y (%d) > 100, adjusting size_y to %d",
        sy, py, 100.f - py);
    sy = 100.f - py;
  }
  ...
  const Vector2D newSize = {static_cast<int>(monitorSize.x * (sx / 100.f)),
...
static SDispatchResult dispatchAddWindow(std::string window) {
  ...
  if (!pWindow)
    return SDispatchResult{.success = false,
                           .error =
                               "[hyprwinwrap] Could not find target window"};

  configureWindow(pWindow);

  auto rule = makeWindowRule(
      pWindow->getPID(), Desktop::Rule::RULE_PROP_EXEC_PID, pWindow->getPID());
...
void onNewWindow(PHLWINDOW pWindow) {
  static auto *const PCLASS =
      (Hyprlang::STRING const *)HyprlandAPI::getConfigValue(
          PHANDLE, "plugin:hyprwinwrap:class")
          ->getDataStaticPtr();
  static auto *const PTITLE =
      (Hyprlang::STRING const *)HyprlandAPI::getConfigValue(
          PHANDLE, "plugin:hyprwinwrap:title")
          ->getDataStaticPtr();
  ...
  const bool classMatches =
      !classRule.empty() && pWindow->m_initialClass == classRule;
...
static SDispatchResult dispatchFreeWindow(std::string window) {
  ...
  for (auto &bg : bgWindows) {
    ...
    std::erase_if(bgRules, [bgw](const auto &rule) {
      if (rule.name() != std::to_string(bgw->getPID()))
        return false;
      Desktop::Rule::ruleEngine()->unregisterRule(rule);
      return true;
    });
...
void onRenderStage(eRenderStage stage) {
  ...
  for (auto &bg : bgWindows) {
    ...
    // cant use setHidden cuz that sends suspended and shit too that would be
    // laggy
    bgw->m_hidden = false;

    g_pHyprRenderer->renderWindow(
        bgw, g_pHyprRenderer->m_renderData.pMonitor.lock(), Time::steadyNow(),
        false, RENDER_PASS_ALL, false, true);
...
void onConfigReloaded() {
  ...
  const std::string classRule(*PCLASS);
  if (!classRule.empty()) {
    auto rule = makeWindowRule("hyprwinwrap-class",
                               Desktop::Rule::RULE_PROP_CLASS, classRule);
    bgRules.emplace_back(rule);
    Desktop::Rule::ruleEngine()->registerRule(SP<Desktop::Rule::IRule>{rule});
  }
  ...
  const std::string titleRule(*PTITLE);
  if (!titleRule.empty()) {
    auto rule = makeWindowRule("hyprwinwrap-title",
                               Desktop::Rule::RULE_PROP_TITLE, titleRule);
    bgRules.emplace_back(rule);
    Desktop::Rule::ruleEngine()->registerRule(SP<Desktop::Rule::IRule>{rule});
  }
...
APICALL EXPORT PLUGIN_DESCRIPTION_INFO PLUGIN_INIT(HANDLE handle) {
  ...
  if (HASH != CLIENT_HASH) {
    HyprlandAPI::addNotification(
        PHANDLE,
        "[hyprwinwrap] Failure in initialization: Version mismatch (headers "
  ...
  static auto P = Event::bus()->m_events.window.open.listen(
      [&](PHLWINDOW w) { onNewWindow(w); });
  static auto P2 = Event::bus()->m_events.window.close.listen(
      [&](PHLWINDOW w) { onCloseWindow(w); });
  static auto P3 = Event::bus()->m_events.render.stage.listen(
  ...
  auto fns = HyprlandAPI::findFunctionsByName(
      PHANDLE, "_ZN7Desktop4View11CSubsurface8onCommitEv");
  if (fns.size() < 1)
  ...
  fns = HyprlandAPI::findFunctionsByName(
      PHANDLE, "_ZN7Desktop4View7CWindow12commitWindowEv");
  if (fns.size() < 1)
  ...
  bool success = true;
  success =
      success && HyprlandAPI::addDispatcherV2(PHANDLE, "hyprwinwrapSetWindow",
                                              ::dispatchSetWindow);
  success =
      success && HyprlandAPI::addDispatcherV2(PHANDLE, "hyprwinwrapFreeWindow",
                                              ::dispatchFreeWindow);

  if (!success)
    throw std::runtime_error("[hyprwinwrap] Dispatchers registration failed.");

  HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprwinwrap:class",
                              Hyprlang::STRING{"kitty-bg"});
  HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprwinwrap:title",
                              Hyprlang::STRING{""});

  HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprwinwrap:size_x",
                              Hyprlang::STRING{"100"});
  HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprwinwrap:size_y",
                              Hyprlang::STRING{"100"});
  HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprwinwrap:pos_x",
                              Hyprlang::STRING{"0"});
  HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprwinwrap:pos_y",
                              Hyprlang::STRING{"0"});

  onConfigReloaded();

  HyprlandAPI::addNotification(PHANDLE,
                               "[hyprwinwrap] Initialized successfully!",
                               CHyprColor{0.2, 1.0, 0.2, 1.0}, 5000);

  return {"hyprwinwrap", "A clone of xwinwrap for Hyprland", "Vaxry", "1.0"};
}

APICALL EXPORT void PLUGIN_EXIT() { clearWindowRules(); }
